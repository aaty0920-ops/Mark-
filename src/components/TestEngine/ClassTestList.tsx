...
const ClassTestList: React.FC<ClassTestListProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<"Upcoming" |"Attempted" |"Missed"
  >("Upcoming");

  const filteredTests = mockClassTests.filter((test) => {
    if (activeTab ==="Upcoming")
      return test.status ==="Not Attempted" && !test.isMissed;
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 pb-32 font-sans relative overflow-hidden">
      ...
      {/* Header */}
      <header className="px-6 py-6 pt-10 sticky top-0 bg-white/8 dark:bg-slate-900/80 backdrop-blur-xl z-40 border-b border-slate-200/60 shadow-sm relative">
        <div className="flex items-center gap-4 mb-4">
          <button
            onClick={onBack}
            className="w-10 h-10 flex items-center justify-center -ml-2 rounded-full hover:bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 transition-colors"
          >
            <ArrowLeft size={22} strokeWidth={2.5} />
          </button>
          <div className="flex items-center gap-3 text-2xl font-bold font-serif text-slate-800 dark:text-white">
        ...
        <div className="flex items-center justify-between mb-6">
          <p className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest pl-[52px]">
            <span className="text-purple-600 font-bold">
              {
                mockClassTests.filter(
                  (t) => !t.isMissed && t.status !=="Attempted",
                ).length
              }
            </span>{""}
            Active Assignments
            ...
            <button
              ...
              className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all relative ${
                activeTab === tab
                  ...
                  :"text-slate-500 dark:text-slate-400 hover:text-slate-700 hover:bg-slate-200/50"
              }`}
            >
              ...
              {activeTab === tab && (
                <motion.div
                  layoutId="active-tab"
                  className="absolute inset-0 border border-slate-200 dark:border-slate-700 rounded-xl"
      ...
      <main className="px-6 mt-6 space-y-5 relative z-10">
        <AnimatePresence mode="popLayout">
          {filteredTests.length > 0 ? (
            filteredTests.map((test) => {
              ...
              return (
                <motion.div
                  ...
                  className={`p-6 rounded-[2rem] border relative overflow-hidden bg-white dark:bg-slate-800/50 backdrop-blur-xl shadow-sm hover:shadow-md transition-shadow group ${
                    test.isLocked && !test.isMissed ?"opacity-80" :""
                  } ${test.status ==="Attempted" ?"border-emerald-200" : test.isMissed ?"border-rose-200" :"border-slate-200 dark:border-slate-700 hover:border-slate-300"}`}
                >
                  <div
                    className={`absolute -right-16 -top-16 w-40 h-40 rounded-full blur-3xl opacity-30 ${
                      colorTheme ==="purple"
                        ?"bg-purple-300"
                        : colorTheme ==="emerald"
                          ?"bg-emerald-300"
                  ...
                  <div className="relative z-10 flex flex-col h-full justify-between">
                    <div>
                      <div className="flex justify-between items-start mb-4">
                        <div>
                          <div className="flex items-center gap-2 mb-3">
                            <span
                              className={`px-2.5 py-1 text-[9px] font-bold rounded uppercase tracking-widest border ${colorConfig[colorTheme]}`}
                            >
                              {test.exam}
                            </span>
                            {test.isLocked && !test.isMissed && (
                              <span className="flex items-center gap-1 text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest bg-white dark:bg-slate-900 px-2.5 py-1 rounded border border-slate-200 dark:border-slate-700">
                                <Lock size={10} strokeWidth={2.5} /> Locked
                              </span>
                            )}
                          </div>
                          <h3 className="text-xl font-bold tracking-tight text-slate-800 dark:text-slate-200 leading-snug mb-2 pr-6 group-hover:text-purple-700 dark:group-hover:text-purple-400 transition-colors">
                            {test.name}
                          </h3>
                        ...
                        {test.status ==="Attempted" ? (
                          ...
                          <div
                            className={`text-right text-[10px] font-bold px-3 py-2 rounded-xl border shadow-sm shrink-0 ${
                              test.isMissed
                                ?"bg-rose-50 text-rose-600 border-rose-100"
                                :"bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-700"
                            }`}
                          >
                            <div className="uppercase tracking-widest mb-1 opacity-70">
                      ...
                      <div className="flex items-center gap-4 mb-6 text-xs text-slate-600 dark:text-slate-400 font-semibold bg-white dark:bg-slate-900 p-3.5 rounded-[1rem] border border-slate-100">
                      ...
                      {test.status ==="Attempted" ? (
                        <>
                          <button
                            ...
                            className="flex-1 py-3.5 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold text-sm rounded-xl flex items-center justify-center gap-2 hover:bg-slate-200 transition-all border border-slate-200 dark:border-slate-700"
                          >
                        ...
                        <button
                          disabled
                          className="w-full py-3.5 bg-white dark:bg-slate-900 text-slate-500 dark:text-slate-400 font-bold text-sm rounded-xl flex items-center justify-center gap-2 border border-slate-200 dark:border-slate-700 cursor-not-allowed shadow-sm"
                        >
                        ...
                        >
                          <div className="absolute inset-0 bg-white/2 dark:bg-slate-900/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-out" />
            ...
            >
              <div className="w-24 h-24 bg-white dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-6 shadow-sm border border-slate-200 dark:border-slate-700 relative">
                {activeTab ==="Upcoming" ? (
                  <Calendar
                    size={40}
                    strokeWidth={1.5}
                ...
                <div className="absolute top-0 right-0 w-6 h-6 bg-white dark:bg-slate-800 rounded-full flex items-center justify-center border-2 border-slate-100 dark:border-slate-700 text-slate-500 dark:text-slate-400 text-xs font-bold shadow-sm">
                  0
                </div>
              </div>
              <h3 className="text-2xl font-semibold font-serif text-slate-800 dark:text-white mb-2">
                {activeTab ==="Upcoming"
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 max-w-[280px] mx-auto leading-relaxed">
                {activeTab ==="Upcoming"
                  ?"Awesome job! You completely cleared your class test backlog. Take a break."
                  : activeTab ==="Attempted"
                    ?"When you finish assigned tests, your performance reports will appear here."
