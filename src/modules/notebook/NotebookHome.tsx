...
const NotebookHome = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-white pb-24 font-kanit">
      <header className="px-6 pt-12 pb-6 sticky top-0 bg-[#0f172a]/90 backdrop-blur-md z-40 border-b border-white/5">
        ...
        {allTags.length > 0 && (
          <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2 items-center">
            <TagIcon size={14} className="text-slate-500 shrink-0 mr-1" />
            {allTags.map(tag => {
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap flex items-center gap-1 transition-all ${
                    isSelected 
                      ...
                      : 'bg-slate-800/30 text-slate-400 border border-white/5 hover:bg-slate-800'
                  }`}
                >
                  {tag}
            ...
            {filteredNotes.map((note, i) => (
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className="text-[10px] text-slate-500 font-medium shrink-0 ml-2">{note.date}</span>
                </div>
                <h3 className="text-lg font-bold text-white mb-2 leading-tight">{note.title}</h3>
                
                {note.tags && note.tags.length > 0 && (
                  <div className="flex flex-wrap gap-2 mb-3">
                    {note.tags.map(tag => (
                      <span key={tag} className="text-[10px] font-bold text-purple-400/80 bg-purple-500/10 px-2 py-0.5 rounded flex items-center gap-1">
                        <TagIcon size={10} />
                        {tag}
                      </span>
                    ))}
                ...
                <p className="text-slate-400 text-sm line-clamp-2 leading-relaxed">{note.content}</p>
              </motion.div>
            ))}
          </AnimatePresence>
        )}
