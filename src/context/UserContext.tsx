...
import { onAuthStateChanged, signInWithPopup, signInWithRedirect, getRedirectResult, GoogleAuthProvider, GithubAuthProvider, signOut, User as FirebaseUser, signInWithEmailAndPassword, createUserWithEmailAndPassword, updateProfile, sendPasswordResetEmail } from 'firebase/auth';
...
interface UserContextType {
  ...
  updateUserProfile: (data: Partial<{ theme: string; notifications: NotificationPrefs; profilePic: string | null; avatarConfig: AvatarConfig | null; dailyGoal: number; dailyTasks: DailyTask[]; tasksDate: string; field: string }>) => Promise<void>;
  field: string;
...
export const UserProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [avatarConfig, setAvatarConfigState] = useState<AvatarConfig | null>(null);
  ...
  const updateUserProfile = async (data: Partial<{ theme: string; notifications: NotificationPrefs; profilePic: string | null; avatarConfig: AvatarConfig | null; dailyGoal: number; dailyTasks: DailyTask[]; tasksDate: string; field: string }>) => {
    if (!user) return;
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      if (firebaseUser) {
        try {
          ...
          unsubActivity = onSnapshot(activityRef, (docSnap) => {
            if (docSnap.exists()) {
              const data = docSnap.data();
          ...
          unsubHistory = onSnapshot(activityQuery, (snapshot) => {
