...
export default function Tests() {
  ...
  const [view, setView] = useState<ViewState>(location.state?.initialView || 'home');
  const [tests, setTests] = useState<CustomTest[]>([]);
  const [activeTestData, setActiveTestData] = useState<any>(null);
  const [testResults, setTestResults] = useState<any>(null);
  const [selectedExam, setSelectedExam] = useState<'JEE Main' | 'NEET' | null>(location.state?.selectedExam || null);
  ...
  const handleFinishTest = (results: any) => {
    ...
    const attempted = results.questions.filter((q: any) => results.answers[q.id] !== undefined).length;
  ...
  return (
    <div className="min-h-screen bg-[#0F172A]">
      ...
      {view === 'class-test-list' && (
        <ClassTestList 
          ...
          onViewAnalysis={(test) => {
             ...
             const mockQuestions = Array(test.questionCount).fill(0).map((_, i) => ({
              id: i.toString(),
              text: `Question ${i + 1}`,
              options: ['A', 'B', 'C', 'D'],
              correctAnswer: 0,
      ...
      {view === 'create-list' && (
        <CreateTestList 
          ...
          onViewAnalysis={(test) => {
            ...
            // Calculate stats for saving
            const attempted = mockQuestions.filter((q: any) => mockAnswers[q.id] !== undefined).length;
            const correct = mockQuestions.filter((q: any) => String(mockAnswers[q.id]) === String(q.correctAnswer)).length;
            const incorrect = attempted - correct;
            const score = correct * 4 - incorrect * 1;
      ...
      {view === 'pyq-chapter-view' && selectedExam && selectedSubject && selectedChapter && (
        <PYQChapterView 
          exam={selectedExam}
      ...
      {view === 'pyq-exam-list' && selectedExam && (
        <PYQExamList 
          ...
          onViewAnalysis={(paper) => {
            const mockQuestions = Array(90).fill(0).map((_, i) => ({
              id: i.toString(),
