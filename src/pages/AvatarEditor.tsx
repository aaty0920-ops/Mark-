...
const CATEGORIES = [
  ...
  { id: 'facialHair', label: 'Facial Hair', hasColor: true, colorKey: 'facialHairColor' },
...
export default function AvatarEditor() {
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col font-kanit text-white">
      {/* Header */}
      <header className="px-4 py-4 flex items-center justify-between sticky top-0 z-10 bg-slate-900/80 backdrop-blur-md">
        <button
          onClick={() => navigate(-1)}
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8 relative min-h-[40vh]">
        <motion.div
          ...
          className="relative w-64 h-64 md:w-80 md:h-80 bg-slate-800 rounded-full shadow-2xl border-4 border-slate-700 overflow-hidden flex items-center justify-center"
        >
        ...
        <div className="absolute bottom-4 left-4 flex gap-2">
          <button
            ...
            className="p-3 bg-slate-800 border border-slate-700 text-slate-300 rounded-full shadow-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-slate-700 transition-colors"
          >
        ...
        <div className="absolute bottom-4 right-4">
          <button
            ...
            className="p-3 bg-slate-800 border border-slate-700 text-slate-300 rounded-full shadow-lg hover:bg-slate-700 transition-colors"
          >
        ...
        {activeCategoryData?.hasColor && activeCategoryData.colorKey && (
          <div className="absolute right-4 top-1/2 -translate-y-1/2 flex flex-col gap-2 bg-slate-800/80 backdrop-blur-md p-2 rounded-full border border-slate-700 max-h-[60%] overflow-y-auto hide-scrollbar">
            {OPTIONS[activeCategoryData.colorKey as keyof typeof OPTIONS].map((color) => (
              <button
                ...
                className={`w-8 h-8 rounded-full border-2 transition-transform ${
                  config[activeCategoryData.colorKey as keyof AvatarConfig]?.[0] === color
      ...
      {/* Editor Controls */}
      <div className="bg-slate-800 border-t border-slate-700 pb-safe rounded-t-3xl">
        {/* Categories */}
        <div className="flex overflow-x-auto hide-scrollbar border-b border-slate-700 px-2 pt-2">
              ...
              return (
                ...
                >
                  ...
                  {isColor && option === 'transparent' && (
                    <div className="absolute inset-0 flex items-center justify-center bg-slate-800 text-xs text-slate-500 font-medium">
                      None
                    </div>
                  )}
                </button>
